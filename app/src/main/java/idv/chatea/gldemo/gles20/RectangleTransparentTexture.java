...
public class RectangleTransparentTexture {
    ...
    /**
     * The keypoint is using '**discard**' when alpha is 0.
     * discard is used to dispose the fragment(pixel).
     *
     * The precondition is, the texture image must has alpha value itself.
     * If the texture image doesn't have alpha value, this will not work.
     * (e.g. *.png image has alpha value, but *.jpg doesn't has alpha value)
     */
    private static final String FRAGMENT_CODE =
            ...
            "varying vec2 vTextureCoord;" +
            "void main() {" +
            "  vec4 color = texture2D(uTextureSample, vTextureCoord);" +
    ...
    public RectangleTransparentTexture(Bitmap bitmap) {
